// svelte_wrapped_device.h - COM wrapper for IDirect3DDevice9 / IDirect3DDevice9Ex
// Mirrors the D3D11 WrappedDevice pattern but for D3D9.
//
// IDirect3DDevice9 has ~127 methods (vs ID3D11Device's ~40). Most are
// mechanical forwards. The methods we intercept:
// - CreateTexture → strip mips (the main VRAM reduction)
// - GetRenderTargetData → (future) handle stripped render targets
//
// QUERYINTERFACE TRICK (same as D3D11):
// The game may QI for IDirect3DDevice9Ex. If we let this fall through to
// the real device, the game gets an unwrapped device and bypasses us.
// So QI returns `this` for IDirect3DDevice9 AND IDirect3DDevice9Ex.
//
// We inherit from IDirect3DDevice9Ex so the vtable covers both interfaces.
#pragma once
#include <d3d9.h>

class WrappedDevice9 : public IDirect3DDevice9Ex {
private:
 IDirect3DDevice9* m_real;
 IDirect3DDevice9Ex* m_realEx; // cached via QI (for IDirect3DDevice9Ex methods)
 LONG m_refCount;

public:
 WrappedDevice9(IDirect3DDevice9* real);
 ~WrappedDevice9();
 ...
 // IDirect3DDevice9 - the method we intercept ──
 ...
 // All other IDirect3DDevice9 methods (forwarded) ──
 // These are declared in the header so the vtable is complete.
 // Implementations are mechanical forwards to m_real.
 ...
 HRESULT STDMETHODCALLTYPE SetVertexShaderConstantB(UINT StartRegister, const BOOL* pConstantData, UINT Vector4bCount) override;
 HRESULT STDMETHODCALLTYPE GetVertexShaderConstantB(UINT StartRegister, BOOL* pConstantData, UINT Vector4bCount) override;
 ...
 HRESULT STDMETHODCALLTYPE SetPixelShaderConstantB(UINT StartRegister, const BOOL* pConstantData, UINT Vector4bCount) override;
 HRESULT STDMETHODCALLTYPE GetPixelShaderConstantB(UINT StartRegister, BOOL* pConstantData, UINT Vector4bCount) override;
 ...
 // IDirect3DDevice9Ex methods (forwarded to m_realEx) ──
 ...
 HRESULT STDMETHODCALLTYPE CheckResourceResidency(IDirect3DResource9** pResourceArray, UINT NumResources) override;
